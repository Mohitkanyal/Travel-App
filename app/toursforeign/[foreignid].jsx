...
export default function ToursInfo() {
  const { foreignid } = useLocalSearchParams();
  const navigation = useNavigation();
  const router = useRouter();
  const [loading,isLoading] = useState(true)
  const [activeIndex, setActiveIndex] = useState(0);
  const [forToursList, setForToursList] = useState([]);
  ...
  const getForToursList = async () => {
    const docRef = doc(db, 'forTours', foreignid);
    const docSnap = await getDoc(docRef);

    if (docSnap.exists()) {
      const data = docSnap.data();
      console.log("ForTours data:", data);
      setForToursList([data]);
      isLoading(false)
      ...
      console.log("No such document in forTours!");
      setForToursList([]);
    }
  };
  if(loading){
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                ...
                onPress={() => router.push(`../bookFor/${foreignid}`)}>
