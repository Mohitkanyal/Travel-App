import { View, Text, TouchableOpacity, StyleSheet, FlatList, Image, useWindowDimensions,ScrollView} from 'react-native';
...
export default function ToursInfo() {
  const { detailid } = useLocalSearchParams(); 
  const navigation = useNavigation();
  const { width, height } = useWindowDimensions();
  const router = useRouter();

  const [indTourList, setIndTourList] = useState([]);
  ...
  const getIndTourDetail = async () => {
    const docRef = doc(db, 'indTours', detailid);
    const docSnap = await getDoc(docRef);

    if (docSnap.exists()) {
      const data = docSnap.data();
      console.log("indTours data:", data);
      setIndTourList([data]); 
      isLoading(false)
      ...
      console.log("No such document in indTours!");
      setIndTourList([]); 
    }
  };
  if(loading){
...
<FlatList
      ...
      renderItem={({ item }) => 
        ...
        <TouchableOpacity
          ...
          onPress={() => router.push(`../bookInd/${detailid}`)}>
