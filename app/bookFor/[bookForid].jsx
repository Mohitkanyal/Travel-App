...
export default function Book() {
  const { bookForid } = useLocalSearchParams();
    ...
    navigation.setOptions({
      ...
      headerLeft: () => (
        <TouchableOpacity onPress={() => navigation.goBack()}>
          <Ionicons name='arrow-back' size={24} style={{ paddingRight: 10 }} />
        </TouchableOpacity>
      ),
    });
  }, [navigation]);
...
if (selectedDate <= currentDate) {
  ...
  return;
    ...
    }, [name, email, phone, travelers, date, price, tripName, auth, router]);
  ...
  const getDetail = async ()=>{
    const docRef = doc(db, 'forTours' , bookForid);
  ...
  return (
    ...
    <View style={{flex:1}}>
      <Text style={{
        position:'absolute',fontSize:50,fontFamily:'outfit-semib',top:'50%',color:'#FAF9F6',left:15,textShadowColor:'black',textShadowOffset:{width:1 , height:2,},textShadowRadius:5}}>
        {item.name}</Text>
